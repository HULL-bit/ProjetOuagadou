...
import { User, Mail, Phone, Ship, FileText, Camera, Save, Lock, Eye, EyeOff, Shield, Bell, Globe, Palette, Upload } from 'lucide-react';
...
const ProfileSettings: React.FC = () => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const avatarPath = `avatars/${user.id}-${Date.now()}.${file.name.split('.').pop()}`;
      const avatarUrl = await uploadFile(file, 'avatars', avatarPath);
